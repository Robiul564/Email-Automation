...
Each scenario includes intent, key facts, tone, and a reference email.
...
gen = EmailGenerator(api_key=None)  # Uses offline mode if no API key

email = gen.generate_email(
    intent="Follow up after meeting",
    key_facts=["Timeline: 6 months", "Budget: $50,000"],
...
evaluator = EmailEvaluator(api_key=None)

results = evaluator.evaluate_email(
...
print(f"Overall: {results['overall_score']:.1f}/100")
print(f"Fact Incorporation: {results['metric_1_fact_incorporation']['score']:.1f}/100")
print(f"Tone Adherence: {results['metric_2_tone_adherence']['score']:.1f}/100")
print(f"Professional Quality: {results['metric_3_professional_quality']['score']:.1f}/100")
```

## Notes

- **Offline mode**: Without an API key, the system uses heuristic scoring. With `OPENAI_API_KEY` set, it uses LLM-as-Judge for more accurate evaluation.
- **Output data**: Check `FINAL_REPORT.md` for deliverables (prompt template, metric definitions, comparative analysis).
